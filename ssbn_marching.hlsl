...
uniform sampler seedTexture;

in float2 texcoord;

layout(location = 0) out float4 outBNAO;

float3 encodeNormal(const in float3 normal)
{
	return normal * 0.5 + 0.5;
}

void checkSSVisibilitySmooth(
	const in float3 csPosition, const in sampler2D smp_position,
	const in float4x4 viewMatrix, const in float4x4 viewProjectionMatrix,
	const in float3 wsSample, const in float outlierDistance,
	inout float visibility, inout bool nonOutlier)
{
	...
	float4 ndcSamplePosition = viewProjectionMatrix * float4(wsSample, 1.0);
	//if(ndcSamplePosition.w == 0.0) continue;
	ndcSamplePosition.xy /= ndcSamplePosition.w;
	float2 tsSamplePosition = ndcSamplePosition.xy * 0.5 + 0.5;

	// optimization: replace position buffer with depth buffer
	// here we get a world space position
	// we found the background...
	float4 wsReferenceSamplePosition = texture(smp_position, tsSamplePosition);
...
void checkSSVisibilityWithRayMarchingSmooth(
	const in float3 csPosition, const in sampler2D smp_position,
	const in float4x4 viewMatrix, const in float4x4 viewProjectionMatrix,
	const in float3 wsPosition, const in float outlierDistance,
	const in float3 ray, const in float sampleRadius,
	...
	inout float visibility, inout bool nonOutlier)
{
	for(int k=0; k<rayMarchingSteps; k++)
	{
		...
		float3 wsSample = wsPosition + ray * (sampleRadius * (float(k) / float(rayMarchingSteps) + rayMarchingStartOffset));
		checkSSVisibilitySmooth(
			csPosition, smp_position,
...
void createOrthoNormalBasis(const in float3 n, out float3 tangent, out float3 binormal, out float3 normal)
{
	normal = normalize(n);

	if(abs(normal.x) > abs(normal.z))
	{
		binormal = float3(-normal.y, normal.x, 0.0);
	}
	else
	{
		binormal = float3(0.0, -normal.z, normal.y);
	}
...
void createOrthoNormalBasisUnsafe(const in float3 n, out float3 tangent, out float3 binormal, out float3 normal)
{
	normal = normalize(n);
...
float4 main() : COLOR
{
	...
	// ts = texture space
	//////////////////////////////////////////////////////////////////////////
	final float4 wsPosition = tex2D(smp_position, ifloat2(gl_FragCoord.xy), 0);
	// background
	if(wsPosition.w == 0) return;
	final float3 csPosition = (viewMatrix * float4(wsPosition.xyz, 1.0)).xyz;
	final float3 wsNormal = tex2D(smp_normal, ifloat2(gl_FragCoord.xy), 0).rgb;
	...
	for(int i=0; i<sampleCount; i++)
	{
		...
		float4 data = tex2D(seedTexture, ifloat2(i, patternIndex), 0);
		float rayMarchingStartOffset = data.a;
		float3 ray = data.rgb;
		//final float3 ray = data.x*u + data.y*v + data.z*w;
	...
	if(dot(bentNormal, float3(1.0)) != 0.0)
	{
